...
class ScanFinActivity : AppCompatActivity() {
    private lateinit var binding: ActivityScanFinBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (dataList != null && dataList.size == 3) {
            val category = dataList[0]
            val name = dataList[1]
            val photoUrl = dataList[2]

            // 데이터 바인딩
            binding.editMedi.text = name?.toEditable()
            ...
            // 데이터를 로그로 확인하거나 UI에 설정
            Log.d("ScanMediActivity", "카테고리: $category, 이름: $name, 사진 URL: $photoUrl")
        }
        ...
        binding.timeSlotRecy.apply {
            layoutManager = LinearLayoutManager(this@ScanFinActivity)
